...
Matrix createLocalK(int e,mesh &m){
    ...
    float tau,kappa, delta, lambda, Ae,J,D;
    
    //Componentes de K
    // [ A+K  -M+W ]
    // [  D   0 ]
    Matrix matrixA,matrixK,matrixM, matrixW,matrixD;
    ...
    productRealMatrix((2*J*lambda)/(18*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixW);

    //Preparando matrixM
    delta = m.getParameter(DELTA);
    productRealMatrix(-(delta*Ae)/(8*(D*D)),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,BPrima,2,2,3),2,2,3),6,2,3),matrixM);
    ...
    ubicarSubMatriz(K,0,5,6,8,sumMatrix(matrixM,matrixW,6,3));
...
Vector createLocalb(int e,mesh &m){
    ...
    float psi_x = m.getParameter(PSI_X);
    float psi_y = m.getParameter(PSI_Y);
